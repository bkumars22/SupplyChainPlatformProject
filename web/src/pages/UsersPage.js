...
import { getUsers, createUser, disableUser } from '../api';
...
export default function UsersPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      await createUser({ userId: form.userId, userName: form.userName, roleName: form.role, password: form.password });
      flash('User created successfully');
      setShowForm(false);
      load();
    } catch(e) {
      flash('Failed to create user: ' + e.message, 'error');
    }
  };
      ...
      {loading ? (
        ...
        <div style={{ background:'#fff', borderRadius:'12px', border:'1px solid #e5e7eb', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'14px' }}>
            ...
            <tbody>
              {users.map((u, i) => {
                const rc = ROLE_COLORS[u.roleName] || ROLE_COLORS.GUEST;
                ...
                return (
                  <tr key={i} style={{ borderBottom:'1px solid #f1f5f9' }}
                    ...
                    <td style={{ padding:'12px 16px' }}>
                      <span style={{ background:rc.bg, color:rc.color, padding:'3px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:'700' }}>{u.roleName}</span>
                    </td>
                    <td style={{ padding:'12px 16px' }}>
                    ...
                    <td style={{ padding:'12px 16px' }}>
                      {isEnabled && (
                        <button onClick={()=>handleDisable(u.userId)} style={{
