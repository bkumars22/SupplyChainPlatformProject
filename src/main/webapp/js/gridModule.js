	...
	for (var i = 0; i < gridColumns.length; i++) {
		...
		if ('string' == typeof colName) {
			...
			var column = {
				...
				"resizeable" : true,
				"removed" : false,
				"menuActions" : [ actionFreeze, actionCollapse, actionRemove ]
			};
			if (expandFlag) {
			...
			if(typeof colName.menuActions == 'undefined')
					colName.menuActions = [ actionFreeze, actionCollapse, actionRemove ];
			
			if(colName.columns) {
				$.each(colName.columns, function( index, value ) {
					if (typeof value.menuActions == 'undefined')
							value.menuActions = [ actionFreeze, actionCollapse, actionRemove ];
				});
			}
...
function createNewGridHandleEvents() {
	if (el != null && typeof el !== 'undefined') {
		...
		/* if any columns are restricted set removed attribute to true for it */
		if(restrictedColumnMap.size > 0)
		...
		gridExpandCallBack();
		grid.alignRows();
        if(!(typeof error_msg_with_rowIndex == 'undefined') && error_msg_with_rowIndex.length > 0){
        	$.each(error_msg_with_rowIndex, function (i, val) {
				buildErrorMessageByTr(val.rowIndex,val.colIndex,val.errorMsg);
       		});
       	}
	}
}

function hideRestrictedColumns() {
		itemGridTemplate.columns = itemGridTemplate.columns.filter(function(column) {
			return 'undefined' != typeof column && !restrictedColumnMap.has(column.name);
		});
		/*console.log('removedColumns are : ' + JSON.stringify(itemGridTemplate.columns));*/
}

function gridExpandCallBack(){
	if($('div.eto-grid-expand__container').length!=0){
	    $('.eto-grid-expand__content').each(function(){
			$(this).css( "overflow-wrap", "anywhere");
			$(this).css( "white-space", "pre-line");
	        if($(this).html().length != 0){
	            $(this).closest( "td").removeClass("eto-grid-expand--expanded");
	        }
			if($(this).parent().children( ".eto-grid-expand__truncated" ).html().length == 0){
			var value = $(this).html();
			$(this).parent().children( ".eto-grid-expand__truncated" ).html(value);
	        }
	    });
	    }
	$('td.eto-grid-expand .eto-grid-expand__toggle').each(function(){
		$(this).css("justify-content", "center");
	});
}
function buildErrorMessageByTr(rowIndex, colIndex, msg) {
	const tr = document.querySelectorAll('#grid-result .eto-grid .eto-grid-scroll table tbody tr')[rowIndex];
	if (!tr) {
		return;
	}
	...
	if (cell) {
		grid.setCellMessage(cell, msg, 'error');
	}
}