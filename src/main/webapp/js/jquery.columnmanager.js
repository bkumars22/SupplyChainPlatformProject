/*
 * NOTE: BLASKO - Customized version to all support for cookie duration
...
(function($) 
{
	var defaults = {
		...
		offClass : '',
		hideInList: [],
		disableIfClass:null, //BLASKO-Customization - do not show in list if class is on column
		colsHidden: [],
		saveState: false,
		mouseOverTitle:null,//BLASKO - Customization - title on fly over
		saveDuration:null, 	//BLASKO - Customization - save cookie for how long
		stateCookieId:null, // BLASKO-Customization - name to use for the cookie that stores state
		stateFieldId:null, // BLASKO-Customization - id of a field to store the state in, can be used to post to server
		onToggle: null,
		show: function(cell){
			showCell(cell);
		},
	...
	var saveCurrentValue = function(table)
	{
		...
		if ( table.cMSaveState)
		{
			for ( ; i < colsVisible.length; i++ )
			{
				val += ( colsVisible[i] == false ) ? 0 : 1;
			}
			//BLASKO - Customization
			if (table.cMCookieId && colsVisible && $.cookie )
			{
			    $.cookie(cookieName + table.cMCookieId, val, {expires: table.cMSaveDuration});
			}
			if (table.cMStateFieldId != null)
			{
			   $('#'+table.cMStateFieldId).val(val);
			}
		}
	};
	...
	$.fn.columnManager = function(options)
	{
		...
		var createColumnHeaderList = function(table)
		{
			...
			for ( var i = 0; i < cells.length; i++ )
			{
				...
				// BLASKO-Customization Skip if hideClass
				if (settings.disableIfClass != null && $(cells[i]).hasClass(settings.disableIfClass))
				{
					continue;
				}
				...
				{
					$(cells[i]).title = settings.mouseOverTitle;
				}
		 ...
		 */
		var getSavedValue = function(table)
		{
			var val = null;
			if (table.cMStateFieldId != null)
			{
				val = $('#'+table.cMStateFieldId).val();		
				val = $.trim(val);
			}
			if (val == null || val.length == 0)
			{
				val = $.cookie(cookieName + table.cMCookieId);
				val = $.trim(val);
			}
        ...
        return this.each(function()
        {
			...
			//BLASKO - Customization
			this.cMStateFieldId = settings.stateFieldId;
			this.cMSaveDuration = settings.saveDuration;
			this.cMCookieId = (settings.stateCookieId != null) ? settings.stateCookieId: this.id;
			this.cMColsHide = colsHide;
			
			//get saved state - and overwrite defaults
			if ( settings.saveState )
			{
				var colsSaved = getSavedValue(this); // BLASKO-customized 
	...
	//BLASKO - Customization added this method
	$.fn.showAllColumns = function(cmo)	
	{
        return this.each(function() 
        {
        	var cols = [];
        	for (var colIndx = 0; colIndx < this.cMColsVisible.length; colIndx++)
        	{
        		if (this.cMColsVisible[colIndx] != undefined && this.cMColsVisible[colIndx] == false)
        		{
        			cols.push(colIndx+1);
        		}
        	...
        	if (cols.length > 0)
        	{
            	$(this).showColumns(cols,cmo);        		
        	}
        	if (cmo.clearSavedState)
        	{
        		$.cookie(cookieName + this.cMCookieId, '', {expires: -1});
        	}
