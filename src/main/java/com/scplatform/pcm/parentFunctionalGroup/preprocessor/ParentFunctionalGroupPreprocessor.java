...
/**
 * ParentFunctionalGroupPreprocessor processes a Parent Functional Group Excel file
 * and converts it into an XML format for the loader.
 */
@Log4j2
@Service
@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class ParentFunctionalGroupPreprocessor extends ExcelToXMLProcessor {
    ...
    public void processReadExcel(Map<String, String> loadProps, String uploadType,
        ...
        writeXml(outFile, "ParentFunctionalGroupMessage.xsd", errors);
    ...
    public void writeXmlHeader(XMLStreamWriter xw) throws XMLStreamException {
        ...
        xw.writeCharacters("\n");
        xw.writeStartElement("scplatform", "ParentFunctionalGroupMessage", E2OPEN_NS);
        ...
        xw.writeAttribute("messageType", "ParentFunctionalGroup");
    ...
    public void writeXml(File outFile, String xsdFileName, Set<String> error) throws IOException, MessageLoaderException {
        ...
        try (BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(outFile))) {
            ...
            for (Map<String, String> row : excelRows) {
                writer.writeStartElement("scplatform", "ParentFunctionalGroup", E2OPEN_NS);
                putAttr(writer, "ParentName",          row.get("ParentName"));
                putAttr(writer, "FunctionalGroupName", row.get("FunctionalGroupName"));
                // Label row col "Action" normalizes to key "Action" (not "OpCode")
                String opCode = row.get("OpCode") != null ? row.get("OpCode")
                              : row.get("opCode") != null ? row.get("opCode")
                              : row.get("Action");
                putAttr(writer, "OpCode",              opCode);
                putAttr(writer, "Description",         row.get("Description"));
                // Label row col "Parent Type" normalizes to key "ParentType"
                String type = row.get("Type") != null ? row.get("Type") : row.get("ParentType");
                putAttr(writer, "Type",                type);
                putAttr(writer, "Purpose",             row.get("Purpose"));
                // Label row col "New Parent Name" normalizes to key "NewParentName"
                String newName = row.get("Name") != null ? row.get("Name") : row.get("NewParentName");
                putAttr(writer, "Name",                newName);
                writer.writeEndElement(); // ParentFunctionalGroup
                writer.writeCharacters("\n");
            }

            writer.writeEndElement(); // ParentFunctionalGroupMessage
        ...
        try {
            ...
            reader.setErrorHandler(new DefaultHandler() {
                private String withContext(SAXParseException e, String level) {
                    ...
                    return SCPlatformMessages.INSTANCE.getMessage("pcm.mdm.pfg.validation.message", arg.toArray(), null);
                }
