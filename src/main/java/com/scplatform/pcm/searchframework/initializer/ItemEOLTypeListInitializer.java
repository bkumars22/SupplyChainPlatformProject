...
public class ItemEOLTypeListInitializer implements SearchParameterInitializer {
  ...
  public boolean initializeParameter(SearchParameter parameter, Map context) {
      if (parameter instanceof SearchParameterSelect) {
          ...
          List<String> itemEOLKeyList = pcmConfigUtil.getList("pcm.itemEOL.type.keys", new ArrayList<String>());
          for (String itemEOLKey : itemEOLKeyList) {
            if(itemEOLKey.equalsIgnoreCase("ACTIVE")) {
              list.addSelectValue(itemEOLKey, null);
            }else {
              list.addSelectValue(itemEOLKey, itemEOLKey);
            }
          }
          return true;
      } else {
