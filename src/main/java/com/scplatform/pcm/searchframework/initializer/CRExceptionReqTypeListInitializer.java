...
public class CRExceptionReqTypeListInitializer implements SearchParameterInitializer {
	...
	public boolean initializeParameter(SearchParameter parameter, Map context) {
		...
		if (parameter instanceof SearchParameterSelect)
		{
			...
			List<String> configuredRequesttTypes = pcmConfigUtil.getList("pcm.costrecord.exception.allowableRequestTypes", new ArrayList<String>());
			
			for (String requestType: configuredRequesttTypes)
			{
				String rstType =  pcmConfigUtil.getString("pcm.costrecord.exception.request.type."+requestType,requestType);
				list.addSelectValue(rstType,requestType);
			}
