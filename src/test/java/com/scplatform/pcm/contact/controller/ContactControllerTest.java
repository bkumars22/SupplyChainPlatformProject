...
class ContactControllerTest {
    ...
    @Test
    void init_returnsContactAdminView_andCallsSearchInit() throws Exception {
        ContactAdminForm form = new ContactAdminForm();
        String view = controller.init(form, request, response, model);
        assertEquals("/admin/contactAdmin", view);
        verify(model).addAttribute("contactAdminForm", form);
        ...
        assertEquals("SearchDefContact.xml", cap.getValue().get("definition"));
    }

    @Test
    void search_mergesAndCallsSearch() throws Exception {
        ContactAdminForm form = new ContactAdminForm();
        ContactAdminForm merged = new ContactAdminForm();
        ...
        assertEquals("/admin/contactAdmin", view);
        verify(model).addAttribute("contactAdminForm", merged);
